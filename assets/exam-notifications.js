(async function () {
  ...
  try {
    ...
    function render() {
      ...
      const placeholders = sources.filter(([key])=>!entries.some(item=>item.category===key));
      area.innerHTML = entries.map(notificationHTML).join('') + placeholders.map(([,body])=>notificationPlaceholderHTML(body.name)).join('');
